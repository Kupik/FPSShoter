...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // disable action use prevent erros
            action.Disable();
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        // verfici duplicatele
                        if(CheckDuplicateBiding(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
        ...
        private bool CheckDuplicateBiding(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            InputBinding newBiding = action.bindings[bindingIndex];

            foreach(InputBinding binding in action.actionMap.bindings)
            {
                if(binding.action == newBiding.action)
                {
                    continue;
                }
                if(binding.effectivePath == newBiding.effectivePath)
                {
                    Debug.Log("Duplicate biding found " + newBiding.effectivePath);
                    return true;
                }
            }
            ...
            if (allCompositeParts)
            {
                ...
                {
                    if (action.bindings[i].effectivePath == newBiding.overridePath)
                    {
                        Debug.Log("Duplicate binding found " + newBiding.effectivePath);
                        return true;
                    }
                }
            }
